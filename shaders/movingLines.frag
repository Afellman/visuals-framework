...
void main() {
  ...
  float y = sin(PI * 2.0 * uv.x * u_params[2] +  u_time * 15.0 ) / 10.0;
  // float swirlz = uv.x * distance(uv.x, 0.5 + sin(u_time)) + distance(uv.x, 0.1);
  uv = vec2((uv.x + u_params[1] + (y * u_params[3])),uv.y + u_params[0]); // u_params[0] is offsetting the y to create the lines
