...
class Rainbow extends Sketch {
  ...
  Line = class Line {
    ...
    move() {
      ...
      this.posStart.x = width / 2 + Math.sin(-HALF_PI + rad) * 768;
      this.posStart.y = height / 2 + -Math.abs(Math.sin(rad) * height / 4);

      this.posEnd.x = width / 2 + Math.sin(-HALF_PI + rad) * (768) / this.parent.lineLength;
      this.posEnd.y = height / 2 + -Math.abs(Math.sin(rad) * (height / 4) / this.parent.lineLength);
    }
